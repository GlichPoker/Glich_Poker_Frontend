...
export default function useLocalStorage<T>(
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) {
        // Try to parse as JSON, but fall back to raw value if that fails
        // This handles both old JSON-stringified values and new direct string storage
        try {
          setValue(JSON.parse(stored) as T);
        } catch {
          // If JSON parsing fails, treat it as a raw string value
  ...
  const set = (newVal: T) => {
    ...
    if (typeof window !== "undefined") {
      // Special handling for string type to avoid double quotes
